...
let Variables = {
    ...
    getTrampolineFormInputs: function (ModalID) {
        ...
        this.trampolineFormInput.forEach(function (inputName) {
            ...
            } else if(inputName === 'trampolineDescription'){
                values[inputName] = $('#'+ModalID+' textarea[name=' + inputName + ']').val()
            } else {
                ...
                addTrampoline: function () {
                    ...
                    $.ajax({
                        ...
                        if (response.status === false) {
                            ...
                            $('#createTrampolineModal form textarea').removeClass('is-invalid')
                            Object.keys(response.failed_input).forEach(function (FailedInput) {
                                ...
                                $('#createTrampolineModal form textarea[name=' + FailedInput + ']').addClass('is-invalid')
                            })
                        }
                        if (response.status) {
        ...
        updateTrampoline: {
            ...
            dataForm: {
                ...
                trampolineDescription: {
                    set: function (Value) {
                        $('#updateTrampolineModal textarea[name=trampolineDescription]').val(Value)
                    }
                },
                trampolinePrice: {
                    set: function (Value) {
                    ...
                    $.ajax({
                        ...
                        if (response.status === false) {
                            ...
                            Object.keys(response.failed_input).forEach(function (FailedInput) {
                                ...
                                console.log('response', response.failed_input[FailedInput][0]);
                                $('#updateTrampolineModal form input[name=' + FailedInput + ']').addClass('is-invalid');
                                $('#updateTrampolineModal form textarea[name=' + FailedInput + ']').addClass('is-invalid')
                            })
                        }
                        if (response.status) {
...
let toolTip = {
    init: function () {
        ...
        tippy('#addTrampoline', {
            content: 'Pridėti naują batutą',
            placement: 'top',
        });
    },
    initTable: function (){
        tippy('#trampolineUpdate', {
            content: 'Redaguoti batutą',
            placement: 'top',
        });
        tippy('#trampolineDelete', {
            content: 'Ištrinti batutą',
            placement: 'top',
        });
    }
}
